...
from Models.Rnn.model import Model
...
def train(model, dataset, fold, nOfEpochs, dataset_test=None, group=''):
    ...
    logger = torch_log.TensorBoardLogger('test', group=group, project='lstm', name= str(fold), add_time=True)
...
def run_lstm(hyperParams, datasetDetails, device="cuda"):
