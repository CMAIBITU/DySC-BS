...
class WindowRNN(nn.Module):
    def __init__(self, dim, numHeads, headDim, drop, attnDrop):
        super().__init__()
        rnn = [nn.LSTM(input_size=dim,
                          ...
                          batch_first=True,
                        #   dropout=attnDrop,
                          bidirectional=True) for _ in range(numHeads)]
        self.rnn = nn.ModuleList(rnn)
        self.fc = nn.Sequential(
            nn.Linear(numHeads*headDim, dim),
            # nn.Dropout(drop)
        )
    ...
    def forward(self, x, x_, mask, nW, analysis=False):
        ...
        #     output = torch.stack((output[:,:, :dim], output[:,:, dim:]), dim=-1)
...
class FusedWindowTransformer(nn.Module):
    ...
    def __init__(self, dim, windowSize, shiftSize, receptiveSize, numHeads, headDim, mlpRatio, attentionBias, drop, attnDrop):
        ...
        self.attention = WindowRNN(dim, numHeads, headDim, drop, attnDrop)
    ...
    def forward(self, x, cls, windowX, windowX_, mask, nW, analysis=False):
        """
            ...
            x : (B, T, C)
            cls : (B, nW, C)
            windowX: (B, 1+windowSize, C) (B*nW, 1+windowSize, C)
            windowX_ (B, 1+windowReceptiveSize, C) (B*nW, 1+windowReceptiveSize, C)
        ...
        # WINDOW ATTENTION
        clsTrans, xTrans  = self.attention(self.attn_norm(windowX), self.attn_norm(windowX_), mask, nW, analysis=analysis) # (B*nW, 1+windowSize, C)
...
class BolTransformerBlock(nn.Module):
    ...
    def forward(self, x, cls, analysis=False):
        ...
        xcls = torch.cat([cls.unsqueeze(dim=2), windowedX], dim = 2) # (B, nW, 1+windowSize, C) # 聪明
        ...
        masks = [self.mask_left, self.mask_right] # 就是去除左右的感受野呗
