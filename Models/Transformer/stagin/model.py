...
class ModuleTimestamping(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=1, dropout=0.0, win_size=5):
        super().__init__()
    ...
    def forward(self, t, sampling_endpoints):
        if sampling_endpoints is None:
            return self.rnn(t)[0]
        # out =  self.rnn(t[:sampling_endpoints[-1]])[0] # t, b, c
        # 取窗口最后一个时间步
        return self.rnn(t[:sampling_endpoints[-1]])[0][[p-1 for p in sampling_endpoints]]
        # out = rearrange(out, 't b c -> b c t')
        # out = nn.functional.max_pool1d(out, self.win_size, 1, 0)
        # out = rearrange(out, 'b c t -> t b c')
        # return out
        # 取中间
        # return out[[p - 1 - self.win_size // 2 for p in sampling_endpoints]]

class ModuleTimestampingCNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=1, dropout=0.0, win_size=5):
        super().__init__()
        # self.rnn = nn.GRU(input_dim, hidden_dim, num_layers=num_layers, dropout=dropout)
        self.win_size = win_size
...
class ModelSTAGIN(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, num_heads, num_layers, sparsity, dropout=0.5, cls_token='sum', readout='sero', garo_upscale=1.0, win_size=5, **others):
        super().__init__()
        ...
        self.timestamp_encoder = ModuleTimestamping(input_dim, hidden_dim, hidden_dim, win_size=win_size)
        # self.timestamp_encoder = ModuleTimestampingCNN(input_dim, hidden_dim, hidden_dim, win_size=win_size)
    ...
    def _collate_adjacency(self, a, sparsity, sparse=True):
        ...
        v_list = []
        a = torch.abs(a)
        for sample, _dyn_a in enumerate(a): # [minibatch x time x node x node]
            for timepoint, _a in enumerate(_dyn_a):
                thresholded_a = (_a > self.percentile(_a, 100-sparsity)) # default is 30
    ...
    def forward(self, v, a, t, sampling_endpoints=None):
        # assumes shape [minibatch x time x node x feature] for v, 传入的是一个one hot编码
        ...
        time_encoding = self.timestamp_encoder(t, sampling_endpoints)# t b c, 因为GRU返回是这个order
        time_encoding = repeat(time_encoding, 'b t c -> t b n c', n=num_nodes)# 实际上是b t n c, 就是每一个node都是相同的特征。

        h = torch.cat([v, time_encoding], dim=3)
        h = rearrange(h, 'b t n c -> (b t n) c')
        h = self.initial_linear(h) # 其实就是加上位置编码，但这样就多了这么多参数，真的好吗？
        ...
        for layer, (G, R, T, L) in enumerate(zip(self.gnn_layers, self.readout_modules, self.transformer_modules, self.linear_layers)):
            ...
            reg_ortho += ( matrix_inner / (matrix_inner.max(-1)[0].unsqueeze(-1) + 1e-10) - torch.eye(num_nodes, device=matrix_inner.device)).triu().norm(dim=(1,2)).mean()

            latent = self.cls_token(h_attend)# 就是cls，就是transformer最后结果
            logit += self.dropout(L(latent))

            attention['node-attention'].append(node_attn)
            attention['time-attention'].append(time_attn)
            latent_list.append(latent)
            ts_rep_list.append(h_attend)
        ...
        attention['time-attention'] = torch.stack(attention['time-attention'], dim=1)
        latent = torch.stack(latent_list, dim=1)
        ts_rep = torch.stack(ts_rep_list, dim=-1)
        # print('latent:', latent.shape) # N, n_of_layers, rep_dim
        # exit()
        return logit, attention, latent, reg_ortho, ts_rep
    
class ModelSTFAGIN(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, num_heads, num_layers, sparsity, dropout=0.5, cls_token='sum', readout='sero', garo_upscale=1.0, win_size=5, 
                 freq_segs_num=3,
                 **others):
        super().__init__()
        ms = []
        for i in range(freq_segs_num):
            ms.append(ModelSTAGIN(input_dim=input_dim,
                                  ...
                                  cls_token=cls_token,
                                  readout=readout,
                                  garo_upscale=garo_upscale,
                                  win_size=win_size))
        
        self.models = nn.ModuleList(ms)

    def forward(self, v, adj_freqs, tx_freqs, sampling_endpoints=None):
        logits = []
        attentions = []
        latents = []
        reg_orthos = 0
        for adj, ts, model in zip(adj_freqs, tx_freqs, self.models):
            logit, attention, latent, reg_ortho = model(v, adj, ts, sampling_endpoints)
            logits.append(logit)
            attentions.append(attention)
            latents.append(latent)
            reg_orthos += reg_ortho
        # logits = torch.mean(logits, )
        logits = torch.mean(torch.stack(logits, dim=-1), dim=-1)
        latents = torch.mean(torch.stack(latents, dim=1),dim=1)
        return logits, attentions, latents, reg_orthos
