...
class model_gnn(nn.Module):
    ...
    def forward(self, A, flag=0):
        ...
        if A.dtype != torch.float32:
            ...
            A_ = A
        A_ = torch.transpose(A_, 1, 0) # 637,2,400,400
        s_feature_p = A_[0]
        s_feature_n = A_[1] 
...
class MyHIgcn(nn.Module):
    def __init__(self, in_dim, hidden_dim, graph_adj, num, thr='0', num_feats=50, num_nodes=871, num_classes=2,
                 brain_region_num=116, name='aal', device='cuda', dataset='abide', n_splits=5):
        '''
        num 是k_folder
        '''
        ...
        # path = '/root/kl2/code/tmp/TE-HI-GCN/model/fgcn/ASD_' + self.name + '/fgcn' + str(self.num) + '_0.' + self.thr + '.pth'
        # if path != 'abide':
        #     path = f'/root/kl2/code/tmp/TE-HI-GCN/model/fgcn/{dataset}_' + self.name + '/fgcn' + str(self.num) + '_0.' + self.thr + '.pth'
        
        # path = f'/root/kl2/code/tmp/TE-HI-GCN-kl/model/fgcn/{n_splits}/{dataset}_{self.name}_base'+'/fgcn' + str(self.num) + '_0.' + self.thr + '.pth'
        # self.gnn.load_state_dict(torch.load(path))

        self.gcn1 = DenseGCNConv(in_channels=50, out_channels=128).to(device)
    ...
    def forward(self, nodes, nodes_adj, device='cuda'):
        ...
        # print(embedding.device, nodes_adj.device)
        # print(self.gcn1.weight.device)
        out1 = self.gcn1(embedding, nodes_adj) # 作为特征表示返回
        ...
        pred = out.squeeze()
        out1 = out1.squeeze(dim=0)
        return out1 , pred, cluster_loss
        # return rep , pred, cluster_loss
...
class EHIgcn2(nn.Module):
    def __init__(self, in_dim, hidden_dim, graph_adj1, graph_adj2, graph_adj3, graph_adj4, graph_adj5, graph_adj6,
                 ...
                 num_feats=50, num_classes=2, brain_region_num=116, name='aal', device='cuda', dataset='abide', n_splits=5):
        ...
        self.higcn1 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj1, num=self.num,
                              thr=self.thr1, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        self.higcn2 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj2, num=self.num,
                              thr=self.thr2, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        self.higcn3 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj3, num=self.num,
                              thr=self.thr3, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        self.higcn4 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj4, num=self.num,
                              thr=self.thr4, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        self.higcn5 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj5, num=self.num,
                              thr=self.thr5, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        self.higcn6 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj6, num=self.num,
                              thr=self.thr6, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        self.higcn7 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj7, num=self.num,
                              thr=self.thr7, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        self.higcn8 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj8, num=self.num,
                              thr=self.thr8, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        self.higcn9 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj9, num=self.num,
                              thr=self.thr9, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        self.higcn10 = MyHIgcn(in_dim=self.in_dim, hidden_dim=self.hidden_dim, graph_adj=self.graph_adj10, num=self.num,
                              thr=self.thr10, brain_region_num=self.brain_region_num, name=self.name, dataset=dataset, n_splits=n_splits)
        
        

    def forward(self, nodes, nodes_adj, device='cuda'):

        rep1, out1, cl1 = self.higcn1(nodes, nodes_adj)
        rep2, out2, cl2 = self.higcn2(nodes, nodes_adj)
        rep3, out3, cl3 = self.higcn3(nodes, nodes_adj)
        rep4, out4, cl4 = self.higcn4(nodes, nodes_adj)
        rep5, out5, cl5 = self.higcn5(nodes, nodes_adj)
        rep6, out6, cl6 = self.higcn6(nodes, nodes_adj)
        rep7, out7, cl7 = self.higcn7(nodes, nodes_adj)
        rep8, out8, cl8 = self.higcn8(nodes, nodes_adj)
        rep9, out9, cl9 = self.higcn9(nodes, nodes_adj)
        rep10, out10, cl10 = self.higcn10(nodes, nodes_adj)
        
        rep = rep1+rep2+rep3+rep4+rep5+rep6+rep7+rep8+rep9+rep10
        # rep = torch.cat([rep1, rep2, rep3, rep4, rep5, rep6, rep7, rep8, rep9, rep10], dim=-1)
          
        
        pred = out1+out2+out3+out4+out5+out6+out7+out8+out9+out10 # 这是predict
        
        cl = cl1 + cl2 + cl3 + cl4 + cl5 + cl6 + cl7 + cl8 + cl9 + cl10 # 这是cluster_loss
...
class EHIgcn3(EHIgcn2):
    def __init__(self, in_dim, hidden_dim, graph_adj1, graph_adj2, graph_adj3, graph_adj4, graph_adj5, graph_adj6,
                 graph_adj7, graph_adj8, graph_adj9, graph_adj10, num,
                 thr1='0', thr2='0', thr3='0', thr4='0', thr5='0', thr6='0', thr7='0', thr8='0', thr9='0', thr10='0',
                 num_feats=50, num_classes=2, brain_region_num=116, name='aal', device='cuda', dataset='abide', n_splits=5):
        
        super(EHIgcn3, self).__init__(in_dim=in_dim, hidden_dim=hidden_dim, graph_adj1=graph_adj1, graph_adj2=graph_adj2, graph_adj3=graph_adj3, graph_adj4=graph_adj4, graph_adj5=graph_adj5, graph_adj6=graph_adj6,
                 graph_adj7=graph_adj7, graph_adj8=graph_adj8, graph_adj9=graph_adj9, graph_adj10=graph_adj10, num=num,
                 thr1=thr1, thr2=thr2, thr3=thr3, thr4=thr4, thr5=thr5, thr6=thr6, thr7=thr7, thr8=thr8, thr9=thr9, thr10=thr10,
                 num_feats=num_feats, num_classes=num_classes, brain_region_num=brain_region_num, name=name, device='cuda', dataset=dataset, n_splits=n_splits)
        # self.fc
        
        # self.fc1 = nn.Linear(128, 32).to(device)
        # self.fc2 = nn.Linear(32, self.num_classes).to(device)
        
        # self.fc1_ls = nn.ModuleList([ nn.Linear(128, 32) for _ in range(10)])
        # self.relu = nn.ReLU()
        self.fc2 = nn.Linear(320, 2)
        self.to(device)
